...
const Storage = {
  ...
  add(item) {
    ...
    // 标记为修改（增量同步用）
    if (window.IDB) IDB.markAsModified(item._id);
...
const ThemeManager = {
  ...
  themes: [
    { id: 'default', name: '无界原白', primary: '#007AFF', bg: '#FFFFFF' },
    { id: 'dark', name: '深邃黑曜', primary: '#0A84FF', bg: '#1C1C1E' },
    { id: 'forest', name: '静谧森林', primary: '#34C759', bg: '#F2F6F0' },
    { id: 'ocean', name: '深海之蓝', primary: '#5AC8FA', bg: '#F0F8FF' },
    { id: 'sunset', name: '黄昏暮色', primary: '#FF9500', bg: '#FFF8F0' }
  ],
  ...
  renderOptions() {
    ...
    container.innerHTML = this.themes.map(theme => `
      <div class="theme-option" data-theme="${theme.id}">
        <div class="theme-preview" style="background: ${theme.bg}; border-color: ${theme.primary}"></div>
...
const App = {
  ...
  showDetail(id) {
    ...
    const isFav = item.isFavorite ? 'filled' : 'none';
